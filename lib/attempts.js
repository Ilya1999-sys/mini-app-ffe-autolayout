...
const TESTER_USERNAMES = new Set(["ilya_uxui_design", "dm1017y", "olya25112"]);

function isTester(username) {
  return typeof username === "string" && TESTER_USERNAMES.has(username.replace(/^@/, "").toLowerCase());
}
...
function attemptLimitApplies(user) {
  return attemptLimitEnabled() && !isTester(user?.username);
}
...
async function getCompletedScore(user) {
  if (!attemptLimitApplies(user)) return null;
  const value = await redisCommand("GET", keyFor(user.id));
...
async function claimCompletion(user, score) {
  if (!attemptLimitApplies(user)) return { claimed: true, enforced: false };
  if (!Number.isInteger(score) || score < 0 || score > 7) throw new Error("Некорректный результат");
  const result = await redisCommand("SET", keyFor(user.id), String(score), "NX");
  return { claimed: result === "OK", enforced: true };
}

module.exports = { isTester, attemptLimitEnabled, attemptLimitApplies, checkAttemptStorage, getCompletedScore, claimCompletion };
