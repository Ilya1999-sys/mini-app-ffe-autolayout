...
const DISCOUNTS_BY_SCORE = [3, 5, 7, 10, 12, 15, 18, 20];

function discountForScore(score) {
  if (!Number.isInteger(score) || score < 0 || score >= DISCOUNTS_BY_SCORE.length) {
    throw new Error("Некорректное количество баллов");
  }
  return DISCOUNTS_BY_SCORE[score];
}
...
module.exports = { CORRECT_ANSWERS, DISCOUNTS_BY_SCORE, discountForScore, scoreAnswers };
